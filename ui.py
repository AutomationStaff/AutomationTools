...
class UVToolsPanel(Panel):
	...
	def draw(self, context):
		...
		column.prop(context.scene, 'apply_uv_scale', text = "Apply UV Scale")
...
class VertexPaintPanel(Panel):
	...
	def draw(self, context):
		...
		column = layout.column(align=True)
		column.label(text = 'Polygons:')
		row = layout.row(align=True)
		row.operator(VertexPaintFill.bl_idname, text = "R").color_new = "Red"
		row.operator(VertexPaintFill.bl_idname, text = "G").color_new = "Green"
		row.operator(VertexPaintFill.bl_idname, text = "B").color_new = "Blue"
		row.operator(VertexPaintFill.bl_idname, text = "WH").color_new = "White"
		row.operator(VertexPaintFill.bl_idname, text = "BL").color_new = "Black"

		column = layout.column(align=True)
		column.label(text = 'Vertices:')
		column.operator(FillVertexColors.bl_idname, text="Fill")
		column.operator(ValueVertexColorsChannels.bl_idname, text="Brightness").channels = (1.0, 1.0, 1.0)

		column.label(text = 'Channels:')
		column.operator(InvertVertexColors.bl_idname, text="Invert")
		column.operator(ConvertAOtoExhaustVertexColors.bl_idname, text="AO to Exhaust Glow").value = 0.2
...
class TransformPanel(Panel):
	...
	def draw(self, context):
		...
		column.label(text = "Transfer:")
		column.operator("object.transfer_transform", text = "Transfer Transform")
		column.separator()
		...
		column.label(text = "Duplicate:")
		column.operator("object.duplicate_offset", text = "Offset")
		column.operator("object.duplicate_circular", text = "Circular")
		column.separator()
		...
		column.label(text = "Get:")
		column.operator(GetUETransforms.bl_idname, text = "Copy UE Transforms")
		column.operator(GetUESplinePoints.bl_idname, text = "Copy UE Spline Points (Single)")
		column.operator(GetCamsoSplinePoints.bl_idname, text = "Copy UE Spline Points (Multiple)")
		row = column.row(align=True)
