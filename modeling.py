...
class ToggleModifiersByType(Operator):
	...
	mod_type : bpy.props.StringProperty(name="type")
...
class UnwrapPipe(Operator):
	...
	def execute(self, context):
		...
		if bm is not None:
			...
			bpy.ops.mesh.create_uvs_by_hard_edges()
...
class VertexPaintFill(Operator):
	bl_idname = "mesh.fill_vertex_colors_polygons"
	bl_label = "Fill  Polygons"
	bl_description = "Fill Polygons"
	bl_options = {'REGISTER', 'UNDO'}
	...
	def replace_vertex_color(self, context):
		...
		if len(sel) > 0:
			for o in sel:
				bpy.context.view_layer.objects.active = o
				obj = o.data
				color = obj.attributes.active_color
...
class InvertVertexColors(Operator):
	...
	def execute(self, context):
		...
		if	mesh.attributes.active_color == None:
			bpy.ops.geometry.color_attribute_add()

		if	mesh.attributes.active_color.data_type != 'BYTE_COLOR' and  mesh.attributes.active_color.domain != 'CORNER':
			bpy.ops.geometry.color_attribute_convert(domain='CORNER', data_type='BYTE_COLOR')

		bm = create_bmesh(self, context)

		color_layer = bm.loops.layers.color.active
		if color_layer == None:
			self.report({'WARNING'},  "Color Attribute is <None>. If it exists, note that only [Face Corner|Byte Color] will work. Try using 'Convert Color Attribute'!")
			bm.free()
			bpy.ops.object.mode_set(mode = 'OBJECT')
			return {'FINISHED'}
		...
		for face in bm.faces:
			for loop in face.loops:
				loop[color_layer][0] = 1.0 - loop[color_layer][0]
				loop[color_layer][1] = 1.0 - loop[color_layer][1]
				loop[color_layer][2] = 1.0 - loop[color_layer][2]
...
class ConvertAOtoExhaustVertexColors(Operator):
	bl_idname = "object.ao_to_exhaust"
	bl_label = "AO to Exhaust"
	bl_description = "Convert AO to Exhaust Vertex Colors"
	bl_options = {'REGISTER', 'UNDO'}
	value: bpy.props.FloatProperty(min=0.0, max=1.0)
	...
	def execute(self, context):
		...
		for face in bm.faces:
			for loop in face.loops:
				loop[color_layer][0] = (1 - loop[color_layer][0]) + self.value
				loop[color_layer][1] = 0.0
				loop[color_layer][2] = 0.0
...
class FillVertexColors(Operator):
	...
	bl_label = "Fill"
	bl_description = "Fill with a new color"
	bl_options = {'REGISTER', 'UNDO'}
	color: bpy.props.FloatVectorProperty(min=0.0, max=1.0)
	...
	def execute(self, context):
		...
		if	mesh.attributes.active_color and mesh.attributes.active_color.data_type != 'BYTE_COLOR' and  mesh.attributes.active_color.domain != 'CORNER':
			bpy.ops.object.mode_set(mode = 'OBJECT')
			bpy.ops.geometry.color_attribute_convert(domain='CORNER', data_type='BYTE_COLOR')
			bpy.ops.object.mode_set(mode = 'EDIT')
...
class ValueVertexColorsChannels(Operator):
	bl_idname = "object.edit_value_vertex_colors_channels"
	bl_label = "Brightness"
	bl_description = "Edit Brightness"
	bl_options = {'REGISTER', 'UNDO'}
	channels: bpy.props.FloatVectorProperty(min=0.0, max=10.0)
	...
	def execute(self, context):
		...
		if len(sel):
			for v in sel:
				for loop in v.link_loops:
					loop[color_layer][0] = self.channels[0] * loop[color_layer][0]
					loop[color_layer][1] = self.channels[1] * loop[color_layer][1]
					loop[color_layer][2] = self.channels[2] * loop[color_layer][2]
		else:
			for v in bm.verts:
				for loop in v.link_loops:
					loop[color_layer][0] = self.channels[0] * loop[color_layer][0]
...
class GenerateIDColors(bpy.types.Operator):
...
class AddEmptyInComponentSelectionCenter(Operator):
	bl_idname = "object.add_empty_in_selection_center"
	...
	bl_options = {'REGISTER', 'UNDO'}	
	empty_name: bpy.props.StringProperty(name='Name', default='SOCKET_')	
	size: bpy.props.FloatProperty(default=1, name='Size')
...
class AddEmptyInLoop(Operator):
	...
	def add_empty(self, context, location, rotation, scale):
		empty = bpy.data.objects.new('SOCKET_' + self.name, None)
...
classes = (
	...
	InvertVertexColors,
	FillVertexColors,
	ConvertAOtoExhaustVertexColors,
	ValueVertexColorsChannels,
	FixMaterialSlots,
	DuplicateOffset,
	DuplicateCircular,
