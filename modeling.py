...
class AT_SnapRotateMeshToTarget(Operator):
	...
	bl_description = 'Rotate mesh to snap to scene cursor. Mesh must have 3 vertices selected and origin in the rotation center. The active vertex is the target, the other two are used for finding rotation axis. Selected vertices create the rotation axis plane'
	bl_options = {'REGISTER', 'UNDO'}
	...
	def execute(self, context):
		...
		target_vertex = bm.select_history[-1].co		
		...
		target_location = matrix.inverted()@cursor.location

		angle = target_location.angle(target_vertex)
		if self.reverse:
			angle = (math.pi*2)-angle
		
		quat = Quaternion(axis,  angle)
		diff = obj.rotation_quaternion.rotation_difference(quat)
		obj.matrix_world = obj.matrix_world@diff.to_matrix().to_4x4()

		self.report({'INFO'},  str(math.degrees((obj.matrix_world.inverted()@cursor.location).angle(bm.select_history[-1].co))) + ' deg')#+ '°'
		bm.free()
...
class AT_SnapRotateVertsToTarget(Operator):
	...
	bl_label = 'Snap Rotate Mesh To Target'
	bl_description = 'Rotate selected vertices to snap to scene cursor. Mesh must have at least 3 selected vertices. The active vertex is the target, the other two are used for finding rotation axis'
	bl_options = {'REGISTER', 'UNDO'}
	reverse: bpy.props.BoolProperty(name='Reverse')
	...
	def execute(self, context):	
		...
		target_vertex = bm.select_history[-1]
		...
		axis = v1.cross(v2).normalized()
		median = calculate_median([vert.co for vert in sel])	

		cursor_position = (matrix.inverted()@cursor.location) - median
		target_vertex_position = target_vertex_position - median
		angle = cursor_position.angle(target_vertex_position)
		
		if self.reverse:
			angle = (math.pi*2)-angle
		
		quat = Quaternion(axis,  angle).inverted()
		for vert in sel:			
			vert.co.rotate(quat)		

		target_vertex_position = target_vertex.co - median		
		angle = math.degrees(cursor_position.angle(target_vertex_position))
		
		self.report({'INFO'},  str(angle) + ' deg')#+ '°'	
...
class AT_NumerateOrdered(AT_ObjectsSelectionHistory):
	...
	def modal(self, context, event):
		if (event.alt and event.type in {'LEFTMOUSE', 'RIGHTMOUSE', 'MIDDLEMOUSE', 'WHEELUPMOUSE', 'WHEELDOWNMOUSE'}) or (event.type in {'MIDDLEMOUSE', 'WHEELUPMOUSE', 'WHEELDOWNMOUSE'}):
			...
			self.stop()
			bpy.ops.object.at_numerate('INVOKE_DEFAULT')
...
class AT_NumerateLODsOrdered(AT_ObjectsSelectionHistory):
	...
	def modal(self, context, event):
		if (event.alt and event.type in {'LEFTMOUSE', 'RIGHTMOUSE', 'MIDDLEMOUSE', 'WHEELUPMOUSE', 'WHEELDOWNMOUSE'}) or (event.type in {'MIDDLEMOUSE', 'WHEELUPMOUSE', 'WHEELDOWNMOUSE'}):
			...
			self.stop()
			bpy.ops.object.at_numerate(lods=True)
			self.selected_objects.clear()
			context.workspace.status_text_set(None)
			return {'FINISHED'}
